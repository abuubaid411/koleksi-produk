...
// Fungsi untuk mendeteksi platform dari URL - SELALU kembalikan 'tiktok' untuk Tokopedia
function detectPlatform(link) {
    if (link.includes('shopee')) return 'shopee';
    // Untuk semua link Tokopedia, kembalikan 'tiktok'
    if (link.includes('tokopedia')) return 'tiktok';
    if (link.includes('tiktok')) return 'tiktok';
    return 'other';
}

// Fungsi untuk mengubah link Tokopedia menjadi TikTok
function convertToTikTokLink(link) {
    if (link.includes('tokopedia')) {
        // Ubah langsung ke format TikTok
        try {
            ...
            if (pathParts.length >= 2) {
                ...
                return `https://www.tiktok.com/@${storeName}/video/${productName}`;
            }
        } catch (e) {
            console.error("Error converting link:", e);
        }
        // Fallback: ganti tokopedia menjadi tiktok
        return link.replace('tokopedia', 'tiktok');
    }
    return link;
}
        ...
        try {
            ...
            let products = rows.map(row => {
                ...
                if (cols.length >= 4) {
                    ...
                    // Ubah link Tokopedia menjadi TikTok
                    const processedLink = convertToTikTokLink(originalLink);

                    // Debug: log untuk memastikan perubahan
                    console.log("Original:", originalLink, "Processed:", processedLink);
                    ...
                    return {
                        ...
                        platform: detectPlatform(processedLink) // Gunakan link yang sudah diubah
                    };
                }
                return null;
            }).filter(Boolean);
...
function renderProducts(hideLoading = false) {
    ...
    if (displayed.length === 0) {
        ...
        displayed.forEach(p => {
            ...
            card.className = `card ${p.platform}`;
            card.innerHTML = `
